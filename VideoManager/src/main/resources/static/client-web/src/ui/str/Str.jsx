...
import {closeCardEditor, openCardEditor} from "../../redux/reducers/cardEditorReducer.js";
...
const Str =({className, style, str, typeStr, idPhrase, indexWord=null, language, isHover,isNativeLang=false,inCardEditor=false})=>{
    ...
    const onClickMicrophone =()=> {
        dispatch(openVoiceEditor({idPhrase, indexWord, str: str?.str, language, typeStr}))
        if(inCardEditor) dispatch(closeCardEditor())
    }
    ...
    return (
        <div className={`str_box2${className}`} style={style}>
            <div className='str_box'>
                ...
                {isHover && ((inCardEditor && isNativeLang)|| !inCardEditor) && <HiOutlineMicrophone className="microphone_str clickable" onClick={onClickMicrophone}/>}
                {isHover && <PlayVoice voiceUuid={str?.voice_uuid} size={small}/>}
                {!str?.card_uuid && isNativeLang && !inCardEditor && <FaFileCirclePlus className='clickable' style={{color: '#e4c546'}} onClick={openCardEditor2}/>}
                {str?.card_uuid && isNativeLang && isHover && !inCardEditor && <FaFilePen className='clickable' onClick={openCardEditor2}/>}
            </div>
            {isNativeLang &&
